...
function AccountPostCard({ post, onClick, onEdit, onDelete, isOwner, isAdmin, language }: { post: any, onClick: () => void, onEdit: (e:any)=>void, onDelete: (e:any)=>void, isOwner: boolean, isAdmin?: boolean, language: string }) {
  ...
  return (
    ...
    >
      <div className="p-4 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/40 border-b dark:border-white/5">
        <div className="flex items-center gap-1 min-w-0">
          <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden relative border-2 border-white dark:border-white/10 shadow-sm shrink-0">
          ...
          <div className="flex items-center gap-1 min-w-0">
            <span className="truncate font-semibold text-xs md:text-sm text-slate-900 dark:text-white">{firstName}</span>
        ...
        <div className="flex gap-1">
           {(isOwner || isAdmin) && (
             <>
                <Button size="icon" variant="ghost" className="h-8 w-8 text-blue-500 rounded-xl" onClick={onEdit}><Edit className="w-4 h-4"/></Button>
                <Button size="icon" variant="ghost" className="h-8 w-8 text-red-500 rounded-xl" onClick={onDelete}><Trash2 className="w-4 h-4"/></Button>
             </>
           )}
        </div>
      </div>
      ...
      <div className="aspect-[4/3] relative bg-slate-900 overflow-hidden">
        ...
        <Badge className="absolute top-4 right-4 bg-primary/90 backdrop-blur-md text-white border-none rounded-xl px-4 py-1.5 text-[10px] font-black uppercase tracking-widest shadow-xl">Lv {post.level || 0}</Badge>
      </div>
      ...
      <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-50 dark:border-white/5">
           <p className="text-xl md:text-3xl font-headline font-bold text-primary tracking-tighter">${parseFloat(post.price?.toString() || '0').toFixed(2)}</p>
           <button className="rounded-xl md:rounded-2xl h-10 md:h-12 px-6 bg-primary text-white font-black text-[10px] md:text-xs shadow-lg uppercase tracking-widest transition-all active:scale-95 flex items-center gap-2">
             {language === 'so' ? 'IIBSO' : 'BUY'} <ArrowRight size={14} />
           </button>
        </div>
      </div>
    </Card>
