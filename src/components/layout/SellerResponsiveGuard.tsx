...
export default function SellerResponsiveGuard() {
  ...
  return (
    <div className="fixed inset-0 z-[10000] bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-500 overflow-y-auto">
       <div className="max-w-lg w-full bg-white dark:bg-slate-900 rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl border-none ring-1 ring-red-500/20">
          {/* Danger Header */}
          <div className="bg-red-600 p-6 md:p-10 text-white relative overflow-hidden">
             <div className="absolute top-0 right-0 p-8 opacity-10"><ShieldAlert size={120} /></div>
             <div className="relative z-10 space-y-3">
                <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl bg-white/20 backdrop-blur-md flex items-center justify-center shadow-inner">
                   <AlertTriangle size={32} className="animate-pulse" />
                </div>
                <div>
                   <h2 className="text-2xl md:text-4xl font-headline font-bold uppercase tracking-tight leading-none">
                      {language === 'so' ? "Arin deg deg ah" : "Security Alert"}
                   </h2>
                   <p className="text-white/60 text-[9px] md:text-xs font-black uppercase tracking-[0.2em] mt-1.5">
          ...
          <div className="p-6 md:p-10 space-y-6 md:space-y-10">
             <div className="space-y-4">
                <p className="text-slate-900 dark:text-white text-sm md:text-lg font-bold leading-relaxed">
                   {language === 'so' 
             ...
             <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2">
                   {stallingPosts.map(p => (
                     <div key={p.id} className="p-3 md:p-4 bg-slate-50 dark:bg-slate-800 rounded-xl md:rounded-2xl flex items-center justify-between border dark:border-white/5">
                        <div className="flex items-center gap-3">
                           <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-slate-200 dark:bg-slate-700 shrink-0 flex items-center justify-center">
                              <Clock className="w-4 h-4 md:w-5 md:h-5 text-red-500" />
                           </div>
                           <div>
                              <p className="text-[10px] md:text-xs font-bold uppercase">{p.gameType} Account</p>
                              <p className="text-[8px] md:text-[9px] font-black opacity-40 uppercase tracking-widest">REF: #{p.id.toUpperCase()}</p>
                           </div>
                        </div>
                        <Badge className="bg-red-500 text-white text-[7px] md:text-[8px] font-black">STALLING</Badge>
                     </div>
                   ))}
                </div>
             </div>
             ...
             <div className="pt-2 flex flex-col gap-3">
                <Button 
                  ...
                  className="w-full h-14 md:h-18 rounded-xl md:rounded-2xl bg-red-600 hover:bg-red-700 text-white font-black text-xs md:text-lg uppercase tracking-widest shadow-xl shadow-red-500/20 active:scale-95 transition-all gap-2"
                >
                   {language === 'so' ? "Hadda xalli" : "Resolve Now"} <ArrowRight size={20} />
                </Button>
                <p className="text-[8px] md:text-[9px] text-center text-slate-400 font-bold uppercase tracking-widest">
