...
export default function ProfileCompletionGuard({ children }: { children: React.ReactNode }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Update Failed", description: "Could not save details." });
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <DialogPrimitive.Root open={open} onOpenChange={() => {}}>
        <DialogPrimitive.Portal>
          ...
          <DialogPrimitive.Content 
            className="fixed left-[50%] top-[50%] z-[100005] grid w-[94%] max-w-md translate-x-[-50%] translate-y-[-50%] border bg-background shadow-2xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] rounded-[2rem] border-none bg-white dark:bg-slate-900 outline-none overflow-hidden max-h-[95vh] overflow-y-auto scrollbar-hide"
          ...
          >
            <div className="bg-primary p-4 sm:p-8 text-white relative overflow-hidden shrink-0">
               ...
               <DialogPrimitive.Description className="text-white/80 text-[8px] sm:text-xs font-medium mt-0.5 uppercase tracking-widest">
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-8 pt-4 space-y-4 sm:space-y-6">
              <div className="flex flex-col items-center">
                 <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-full border-4 border-slate-50 dark:border-slate-800 shadow-lg overflow-hidden relative bg-slate-100 dark:bg-slate-800">
                    {userProfile?.photoURL || user?.photoURL ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-slate-300"><UserCircle size={32} /></div>
                    )}
                 </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  <Label className="text-[9px] font-black uppercase text-slate-400 ml-1 tracking-widest flex items-center gap-2">
                    <UserCircle size={10} /> Display Name
                  </Label>
                  <Input 
                    placeholder="Enter name" 
                    value={formData.name} 
                    onChange={e => setFormData({...formData, name: e.target.value})}
                    className="h-10 sm:h-12 rounded-xl bg-slate-50 dark:bg-slate-800 border-none font-bold px-4 shadow-inner text-sm"
                    required
                  />
                </div>
                ...
                <div className="space-y-1">
                  <Label className="text-[9px] font-black uppercase text-slate-400 ml-1 tracking-widest flex items-center gap-2">
                    <Gamepad2 size={10} /> Player ID / UID
                  </Label>
                  <Input 
              ...
              <Button 
                type="submit" 
                disabled={isSaving}
                className="w-full h-12 sm:h-16 rounded-xl sm:rounded-2xl bg-primary hover:bg-primary/90 text-white font-bold text-sm sm:text-lg shadow-xl shadow-primary/20 transition-all active:scale-95 uppercase tracking-widest mt-2"
              >
