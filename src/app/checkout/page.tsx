...
function CheckoutContent() {
  ...
  const handleBooyahRedirect = () => {
    if (gameDetails.playerName.trim().length < 4) { toast({ title: "Magaca wuu gaabanyahay", variant: "destructive" }); return; }
    if (gameDetails.playerID.length < 5) { toast({ title: "Game ID khaldan", variant: "destructive" }); return; }
    const cleanWhatsapp = gameDetails.whatsappNumber.replace(/\D/g, '');
    const cleanSender = gameDetails.senderNumber.replace(/\D/g, '');
  ...
  const handleFinalConfirm = () => {
    ...
    const finalDetails = { ...gameDetails, gameTitle: game?.title || "Unknown Game", itemTitle: item.title, category: isFreeFire ? "Free Fire" : isBloodStrike ? "Blood Strike" : "General" };
        ...
        <Card className="rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border-none p-0.5 md:p-2 bg-white dark:bg-slate-900">
          ...
          <CardContent className="p-4 md:p-8 pt-0 md:pt-0">
            <form onSubmit={!isBooyahPass ? handleDetailsSubmit : (e) => e.preventDefault()} className="space-y-4 md:space-y-6">
              ...
              <Button type="button" onClick={isBooyahPass ? handleBooyahRedirect : handleDetailsSubmit} className="w-full h-13 md:h-16 rounded-xl md:rounded-2xl text-sm md:text-xl font-bold gap-2 shadow-xl shadow-primary/20 active:scale-95 transition-all">{isBooyahPass ? "iibso" : "Continue to Payment"} {isBooyahPass ? <MessageCircle className="w-4 h-4 md:w-6 md:h-6" /> : <ChevronRight className="w-4 h-4 md:w-6 md:h-6" />}</Button>
            </form>
          </CardContent>
        </Card>
      </div>
                ...
                {paymentMethods.map((method) => (
                  <div key={method.id} onClick={() => setSelectedMethodId(method.id)} className={cn("flex items-center justify-between p-3 md:p-5 border-2 rounded-xl md:rounded-[2rem] cursor-pointer transition-all active:scale-[0.98]", selectedMethodId === method.id ? 'border-primary bg-primary/5 dark:bg-primary/10' : 'border-gray-50 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-slate-800/50')}>
                    <Label htmlFor={method.id} className="flex items-center gap-3 md:gap-4 cursor-pointer w-full">
                      ...
                      <div className="flex-1 min-w-0"><p className="font-bold text-sm md:text-lg dark:text-white truncate">{method.name}</p><p className="text-[8px] md:text-xs text-muted-foreground dark:text-slate-500 font-medium">Fast mobile payment</p></div>
          ...
          <CardContent className="pt-6 md:pt-10">
            ...
            <div className="bg-primary/5 dark:bg-primary/10 p-4 md:p-8 rounded-2xl md:rounded-[2rem] mb-6 md:mb-12 text-left border border-primary/10 dark:border-primary/20 shadow-inner">
              ...
              <div className="space-y-1.5 md:space-y-3 pt-3 md:pt-5 border-t border-primary/10 dark:border-white/5 mt-2">
                ...
                <div className="text-[10px] md:text-[13px] text-muted-foreground dark:text-slate-500 flex justify-between items-center gap-2"><span className="truncate">Player ID:</span><span className="font-mono font-bold text-foreground dark:text-slate-200 shrink-0">{gameDetails.playerID}</span></div>
              </div>
            </div>
            <div className="flex flex-col gap-2.5 md:gap-4">
              <Button onClick={handleFinalConfirm} disabled={isProcessing} className="w-full h-14 xs:h-16 md:h-20 rounded-xl md:rounded-[2.5rem] text-sm xs:text-lg md:text-2xl font-bold shadow-xl shadow-primary/30 active:scale-95 transition-all uppercase tracking-wider">{isProcessing ? <div className="flex items-center justify-center gap-2 md:gap-3"><Loader2 className="w-4 h-4 md:w-6 md:h-6 animate-spin" /><span>Verifying...</span></div> : "Waan Bixiyay (Xaqiiji)"}</Button>
