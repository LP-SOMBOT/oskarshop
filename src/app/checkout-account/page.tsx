...
function CheckoutAccountContent() {
  ...
  const handleContactSeller = async () => {
    ...
    try {
      ...
      toast({ title: "Opening WhatsApp!", description: "La xariir seller-ka hadda." });
    } catch (e: any) {
    ...
    <main className="max-w-xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      {step === 1 && (
        <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4">
          <h1 className="text-2xl sm:text-3xl font-headline font-bold text-center text-slate-900 dark:text-white">Iibso Account</h1>
          ...
          >
            {isProcessing ? <Loader2 className="animate-spin" /> : <><MessageCircle size={20} className="sm:w-[22px] sm:h-[22px]" /> La xariir</>}
          </Button>
        </div>
      )}
